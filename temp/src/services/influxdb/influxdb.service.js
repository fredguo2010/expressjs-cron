  ...
  const resultArray = Object.values(groupedData)
    .map((group) => {
      return group.slice(0, -1).map((startObj, i) => {
        ...
        const calcValue = endObj._value - startObj._value;
        ...
        return {
          ...
          cDate: new Date(endObj._time).toLocaleString().replace(/[-/:]/g, '').slice(0, 8),
          cHour: new Date(endObj._time).toLocaleString().slice(11, 16),
        };
      });
    })
    .flat();
